...
type RunIdentifier struct{ kdb.RunBody }
...
func New(ex *kdb.Run) (*Executable, error) {
	...
	if l := ex.Log; l != nil {
		...
		log = &kdb.Assignment{
			MountPoint:   kdb.MountPoint{Id: l.Id, Path: "/log", Tags: l.Tags},
...
func (r *Executable) Build(conf *bconf.KnitClusterConfig) *kubebatch.Job {
	...
	logs, logsMount := tuple.UnzipPair(utils.Map(
		utils.Default(
			utils.IfNotNil(r.Log, func(log *kdb.Assignment) *[]kdb.Assignment {
				return &[]kdb.Assignment{*log}
			}),
			[]kdb.Assignment{},
		),
		toVolumeMount,
...
func toVolumeMount(a kdb.Assignment) tuple.Pair[kubecore.Volume, kubecore.VolumeMount] {
	v := kubecore.Volume{
