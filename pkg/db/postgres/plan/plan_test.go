...
func TestPlan_Register(t *testing.T) {
	...
	// success case
	theoryOk := func(given tables.Operation, when when, then []*kdb.Plan) func(*testing.T) {
		return func(t *testing.T) {
...

			{
				...
				if !cmp.SliceContentEqWith(actual, then, (*kdb.Plan).Equiv) {
					t.Errorf(
						"All Plans:\n===actual===\n%+v\n===expected===\n%+v",
						actual, then,
					)
			...
			expectedNominatorCalls := [][]int{
				utils.Map(
					...
					func(mp kdb.MountPoint) int { return mp.Id },
				),
			}
			...
			spec: kdb.BypassValidation(
				...
				kdb.PlanParam{
					...
					OnNode: []kdb.OnNode{
						...
						{Mode: kdb.PreferOnNode, Key: "accelarator", Value: "tpu"},
					...
					Inputs: []kdb.MountPointParam{
						...
						},
						{
							...
							Tags: kdb.NewTagSet([]kdb.Tag{
								...
								{Key: kdb.KeyKnitId, Value: th.Padding36("some-knit-id")},
								{Key: kdb.KeyKnitTimestamp, Value: "2022-08-15T12:34:56+00:00"},
							}),
						},
					},
					Outputs: []kdb.MountPointParam{
		...
		[]*kdb.Plan{
			{
				PlanBody: kdb.PlanBody{
					PlanId: th.Padding36("plan-1"), Active: true, Hash: th.Padding64("example-hash"),
					Image: &kdb.ImageIdentifier{Image: "repo.invalid/test-image", Version: "v0.1"},
				},
				Inputs: []kdb.MountPoint{
					{
						Id: 100, Path: "/in/1",
						Tags: kdb.NewTagSet([]kdb.Tag{{Key: "tag-1", Value: "value-1"}}),
					},
				},
				Outputs: []kdb.MountPoint{
					{
						Id: 200, Path: "/out/1",
						Tags: kdb.NewTagSet([]kdb.Tag{{Key: "tag-2", Value: "value-2"}}),
					},
				},
			},
			{
		...
		[]*kdb.Plan{
			{
				PlanBody: kdb.PlanBody{
					PlanId: th.Padding36("plan-3"), Active: true, Hash: th.Padding64("hash:plan-3"),
					Image: &kdb.ImageIdentifier{Image: "repo.invalid/image-3", Version: "0.0.1"},
				},
				Inputs: []kdb.MountPoint{
					{
						Id: 31, Path: "/in/3",
		...
		[]*kdb.Plan{
			{
				PlanBody: kdb.PlanBody{
					PlanId: th.Padding36("plan-1"), Active: true, Hash: th.Padding64("hash:plan-1"),
					Image: &kdb.ImageIdentifier{Image: "repo.invalid/image-1", Version: "0.0.1-alpha"},
				},
				Inputs: []kdb.MountPoint{
					{
						Id: 11, Path: "/in/1",
	...
	// error case
	theoryErr := func(given tables.Operation, when *kdb.PlanSpec, then error) func(*testing.T) {
		return func(t *testing.T) {
			ctx := context.Background()
			...
			}
			{
				...
				for input, attr := range given.Inputs {
					expected = append(expected, utils.Map(
						attr.UserTag, func(tag kdb.Tag) record {
							return record{
				...
				if !cmp.SliceContentEqWith(
					...
					func(a, b record) bool {
						return a.InputId == b.InputId &&
							(&kdb.Tag{Key: a.Key, Value: a.Value}).Equal(
								&kdb.Tag{Key: b.Key, Value: b.Value},
							)
					},
				) {
					t.Errorf(
	...
	t.Run("let a plan given, when it is passed a plan spec which is equivarent to given, it causes ErrEquivPlanExistsAlready", theoryErr(
		tables.Operation{
			...
			Inputs: map[tables.Input]tables.InputAttr{
				{InputId: 1, PlanId: th.Padding36("given-plan"), Path: "/in/1"}: {
					UserTag: []kdb.Tag{{Key: "tag-1", Value: "val-1"}},
				},
			},
			Outputs: map[tables.Output]tables.OutputAttr{
				{OutputId: 2, PlanId: th.Padding36("given-plan"), Path: "/out/1"}: {
					UserTag: []kdb.Tag{{Key: "tag-2", Value: "val-2"}},
				},
			},
		},
		kdb.BypassValidation(
		...
		kdb.NewErrEquivPlanExists(th.Padding36("given-plan")),
	))
...
func TestPlan_Find(t *testing.T) {
	...
	given := tables.Operation{
		...
		Inputs: map[tables.Input]tables.InputAttr{
			{InputId: 1_100, PlanId: th.Padding36("plan-1"), Path: "/in/1"}: {
				UserTag: []kdb.Tag{{Key: "key1", Value: "val1"}},
			},

			{InputId: 2_100, PlanId: th.Padding36("plan-2"), Path: "/in/1"}: {
				UserTag: []kdb.Tag{{Key: "key1", Value: "val2"}},
			},
			{InputId: 2_200, PlanId: th.Padding36("plan-2"), Path: "/in/2"}: {
				UserTag: []kdb.Tag{{Key: "key2", Value: "val1"}},
			},
		...
		Outputs: map[tables.Output]tables.OutputAttr{
			...
			{OutputId: 1_020, PlanId: th.Padding36("plan-1"), Path: "/out/2"}: {
				UserTag: []kdb.Tag{{Key: "key2", Value: "val2"}},
				IsLog:   true,
			},
	...
	t.Run("imageVer", theory(
		When{imageVer: kdb.ImageIdentifier{Image: "repo.invalid/image-a", Version: "v1.0"}},
		Then{
			planIds: []string{
	...
	t.Run("imageVer without tag", theory(
		When{imageVer: kdb.ImageIdentifier{Image: "repo.invalid/image-a", Version: ""}},
		Then{
			planIds: []string{
	...
	t.Run("inTag with a single tag", theory(
		When{inTag: []kdb.Tag{{Key: "key2", Value: "val1"}}},
		Then{
			planIds: []string{
	...
	t.Run("inTag with multiple tags", theory(
		When{inTag: []kdb.Tag{{Key: "key1", Value: "val2"}, {Key: "key2", Value: "val1"}}},
		Then{
			planIds: []string{
				th.Padding36("plan-3"),
			},
	...
	t.Run("inTag with knit#id tag", theory(
		When{inTag: []kdb.Tag{{Key: kdb.KeyKnitId, Value: th.Padding36("knit-1")}}},
		Then{
			planIds: []string{
				th.Padding36("plan-5"),
			},
	...
	t.Run("inTag with timestamp tag", theory(
		When{inTag: []kdb.Tag{{Key: kdb.KeyKnitTimestamp, Value: "2023-05-04T06:07:08Z"}}},
		Then{
			planIds: []string{
				th.Padding36("plan-7"),
			},
	...
	t.Run("outTag with a single tag", theory(
		When{outTag: []kdb.Tag{{Key: "key2", Value: "val1"}}},
		Then{
			planIds: []string{
	...
	t.Run("outTag with multiple tags", theory(
		When{outTag: []kdb.Tag{{Key: "key1", Value: "val1"}, {Key: "key2", Value: "val1"}}},
		Then{
			planIds: []string{
				th.Padding36("plan-5"),
			},
	...
	t.Run("active=false, imageVer, inTag, outTag", theory(
		When{
			active:   logic.False,
			imageVer: kdb.ImageIdentifier{Image: "repo.invalid/image-a", Version: "v1.1"},
			inTag:    []kdb.Tag{{Key: "key2", Value: "val1"}},
			outTag:   []kdb.Tag{{Key: "key1", Value: "val1"}},
		},
		Then{
			planIds: []string{
				th.Padding36("plan-4"),
	...
	t.Run("imageVer, inTag with knit#id, outTag", theory(
		When{
			imageVer: kdb.ImageIdentifier{Image: "repo.invalid/image-b", Version: "v1.1"},
			inTag:    []kdb.Tag{{Key: kdb.KeyKnitId, Value: th.Padding36("knit-2")}},
			outTag:   []kdb.Tag{{Key: "key2", Value: "val2"}},
		},
		Then{
			planIds: []string{
				th.Padding36("plan-6"),
	...
	t.Run("active=active, imageVer, inTag with timestamp", theory(
		When{
			active:   logic.True,
			imageVer: kdb.ImageIdentifier{Image: "repo.invalid/image-c", Version: "v1.0"},
			inTag:    []kdb.Tag{{Key: kdb.KeyKnitTimestamp, Value: "2023-05-04T06:07:08Z"}},
		},
		Then{
			planIds: []string{
				th.Padding36("plan-7"),
...
func TestPlan_Activate(t *testing.T) {
	...
	type then struct {
		plan tables.Plan
		run  map[string]kdb.KnitRunStatus
		err  error
	}
		...
		"Let a plan which has runs without output data be given: ": {
			given: tables.Operation{
				...
				Steps: []tables.Step{
					{
						Run: tables.Run{
							RunId: th.Padding36("run-1-a"), PlanId: th.Padding36("plan-1"), Status: kdb.Done,
					...
					{
						Run: tables.Run{
							RunId: th.Padding36("run-1-b"), PlanId: th.Padding36("plan-1"), Status: kdb.Waiting,
							UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
					...
					{
						Run: tables.Run{
							RunId: th.Padding36("run-2-a"), PlanId: th.Padding36("plan-2"), Status: kdb.Deactivated,
							UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
			...
			testcase: map[string]testcase{
				"[inactive -> active] it should return metadata of the plan after activated": {
					...
					then{
						run: map[string]kdb.KnitRunStatus{
							th.Padding36("run-1-a"): kdb.Done,
							th.Padding36("run-1-b"): kdb.Waiting,
							th.Padding36("run-2-a"): kdb.Waiting,
						},
						plan: tables.Plan{
				...
				"[active -> active] it should do nothing but return metadata of the plan": {
					...
					then{
						run: map[string]kdb.KnitRunStatus{
							th.Padding36("run-1-a"): kdb.Done,
							th.Padding36("run-1-b"): kdb.Waiting,
							th.Padding36("run-2-a"): kdb.Deactivated,
						},
						plan: tables.Plan{
							PlanId: th.Padding36("plan-1"), Active: true, Hash: th.Padding64("xxx-hash-xxx"),
						},
				...
				"[active -> inactive] it should return metadata of plan same after deactivated": {
					...
					then{
						run: map[string]kdb.KnitRunStatus{
							th.Padding36("run-1-a"): kdb.Done,
							th.Padding36("run-1-b"): kdb.Deactivated,
							th.Padding36("run-2-a"): kdb.Deactivated,
						},
						plan: tables.Plan{
		...
		"Let a plan which has runs with output be given: ": {
			given: tables.Operation{
				...
				Steps: []tables.Step{
					{
						Run: tables.Run{
							RunId: th.Padding36("run-1-pseudo-1"), PlanId: th.Padding36("pseudo-plan-1"), Status: kdb.Done,
							UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
								"2022-08-15T12:34:56+00:00",
							)).OrFatal(t).Time(),
						},
					...
					{
						Run: tables.Run{
							RunId: th.Padding36("run-2-pseudo-1"), PlanId: th.Padding36("pseudo-plan-1"), Status: kdb.Done,
							UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
								"2022-08-15T12:34:56+00:00",
							)).OrFatal(t).Time(),
						},
					...
					{
						Run: tables.Run{
							RunId: th.Padding36("run-3-pseudo-1"), PlanId: th.Padding36("pseudo-plan-1"), Status: kdb.Done,
							UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
								"2022-08-15T12:34:56+00:00",
							)).OrFatal(t).Time(),
						},
					...
					{
						Run: tables.Run{
							RunId: th.Padding36("run-4-pseudo-1"), PlanId: th.Padding36("pseudo-plan-1"), Status: kdb.Done,
							UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
								"2022-08-15T12:34:56+00:00",
							)).OrFatal(t).Time(),
						},
			...
			testcase: map[string]testcase{
				"when we deactivate a plan which has runs with output data, it should lock the data": {
					...
					then{
						run: map[string]kdb.KnitRunStatus{
							th.Padding36("run-1-pseudo-1"): kdb.Done,
							th.Padding36("run-2-pseudo-1"): kdb.Done,
							th.Padding36("run-3-pseudo-1"): kdb.Done,
							th.Padding36("run-4-pseudo-1"): kdb.Done,
							th.Padding36("run-1-a"):        kdb.Done,
							th.Padding36("run-1-b"):        kdb.Deactivated,
							th.Padding36("run-2-a"):        kdb.Deactivated,
						},
		...
		for name, testcase := range situation.testcase {
			t.Run(situationName+name, func(t *testing.T) {
				...
				}
				{
					...
					actual := map[string]kdb.KnitRunStatus{}
					for _, r := range runs {
