...
func TestRun_Finish(t *testing.T) {
	...
	t.Run("it should cause ErrMissing when finishing nonexisting run", func(t *testing.T) {
		...
		if err := testee.Finish(ctx, runId); !errors.Is(err, kdb.ErrMissing) {
			t.Errorf(`unexpected error: (actual, expected) = (%+v, %+v)`, err, kdb.ErrMissing)
		}
	})
	...
	t.Run("runs should not be finished if", func(t *testing.T) {
		...
		for name, testcase := range map[string]struct {
			...
			t.Run(name, func(t *testing.T) {
				...
				beforeFinish := try.To(scanner.New[tables.Run]().QueryAll(
					ctx, conn,
					`select * from "run" where "run_id" = $1`,
					testcase.runId,
				)).OrFatal(t)
