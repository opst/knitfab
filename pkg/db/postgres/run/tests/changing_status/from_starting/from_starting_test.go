...
func Test_ChangingStatus_FromStarting(t *testing.T) {
	...
	for _, testcase := range []struct {
	...
	}{
		{
			when: []When{
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-starting-1")],
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Starting},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-starting-X"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-starting-2")],
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Starting},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-starting-1"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@image-starting-1")],
					Cursor: kdb.RunCursor{
						Status:   []kdb.KnitRunStatus{kdb.Starting},
						Head:     th.Padding36("run@image-starting-X"),
						Debounce: time.Hour,
					},
				},
			},
			change: []Expectation{
				...
				{
					Statuses: []kdb.KnitRunStatus{
						kdb.Waiting, kdb.Deactivated, kdb.Failed, kdb.Done, kdb.Invalidated,
					},
