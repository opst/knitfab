...
func Test_ChangingStatus_FromAborting(t *testing.T) {
	...
	for _, testcase := range []struct {
	...
	}{
		{
			when: []When{
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-aborting-1")],
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Aborting},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-aborting-X"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-aborting-2")],
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Aborting},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-aborting-1"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@image-aborting-1")],
					Cursor: kdb.RunCursor{
						Status:   []kdb.KnitRunStatus{kdb.Aborting},
						Head:     th.Padding36("run@image-aborting-X"),
						Debounce: time.Hour,
					},
				},
			},
			change: []Expectation{
				...
				},
				{
					Statuses: []kdb.KnitRunStatus{
						...
						kdb.Completing, kdb.Done, kdb.Invalidated,
					},
