...
func Test_ChangingStatus_FromDone(t *testing.T) {
			...
			when: []When{
				{
					Cursor: kdb.RunCursor{
						...
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-done-X"),
						Debounce:   time.Hour,
					},
					Target: given.ExpectedRun[th.Padding36("run@pseudo-done-1")],
				},
				{
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Done},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-done-1"),
						Debounce:   time.Hour,
					},
					Target: given.ExpectedRun[th.Padding36("run@pseudo-done-2")],
				},
				{
					Cursor: kdb.RunCursor{
						Head:     th.Padding36("run@image-done-X"),
						Status:   []kdb.KnitRunStatus{kdb.Done},
						Debounce: time.Hour,
					},
					Target: given.ExpectedRun[th.Padding36("run@image-done-1")],
				},
			},
			change: []Expectation{
				{
