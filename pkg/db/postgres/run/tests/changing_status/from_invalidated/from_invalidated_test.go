...
func Test_ChangingStatus_FromInvalidated(t *testing.T) {
			...
			when: []When{
				{
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Invalidated},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-invalidated-X"),
						Debounce:   time.Hour,
					},
					Target: given.ExpectedRun[th.Padding36("run@pseudo-invalidated-1")],
				},
				{
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Invalidated},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-invalidated-1"),
						Debounce:   time.Hour,
					},
					Target: given.ExpectedRun[th.Padding36("run@pseudo-invalidated-2")],
				},
				{
					Cursor: kdb.RunCursor{
						Status:   []kdb.KnitRunStatus{kdb.Invalidated},
						Head:     th.Padding36("run@image-invalidated-X"),
						Debounce: time.Hour,
					},
					Target: given.ExpectedRun[th.Padding36("run@image-invalidated-1")],
				},
			},
			change: []Expectation{
				{
					Statuses: []kdb.KnitRunStatus{
						kdb.Waiting, kdb.Deactivated, kdb.Ready, kdb.Starting, kdb.Running,
						kdb.Aborting, kdb.Completing, kdb.Failed, kdb.Done, kdb.Invalidated,
					},
