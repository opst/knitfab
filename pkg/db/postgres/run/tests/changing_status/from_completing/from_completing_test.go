...
func Test_ChangingStatus_FromCompleting(t *testing.T) {
	...
	for _, testcase := range []struct {
	...
	}{
		{
			when: []When{
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-completing-1")],
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Completing},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-completing-X"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-completing-2")],
					Cursor: kdb.RunCursor{
						Status:     []kdb.KnitRunStatus{kdb.Completing},
						Pseudo:     []kdb.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-completing-1"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@image-completing-1")],
					Cursor: kdb.RunCursor{
						Status:   []kdb.KnitRunStatus{kdb.Completing},
						Head:     th.Padding36("run@image-completing-X"),
						Debounce: time.Hour,
					},
				},
			},
			change: []Expectation{
				...
				},
				{
					Statuses: []kdb.KnitRunStatus{
						...
						kdb.Aborting, kdb.Failed, kdb.Invalidated,
					},
