...
func TestNominator_NominateData(t *testing.T) {
						...
						Outcomes: map[tables.Data]tables.DataAttibutes{
							{
								...
								Timestamp: &DAY_1, UserTag: []kdb.Tag{},
							},
						},
					},
				},
	...
	for _, nonDone := range []kdb.KnitRunStatus{
		kdb.Deactivated, kdb.Waiting, kdb.Ready,
		kdb.Starting, kdb.Running, kdb.Aborting, kdb.Completing,
	...
	for name, testcase := range map[string]struct {
		...
		"if input has user tags but no data have such, it should not nominate the input": {
			given: tables.Operation{
				...
				Inputs: map[tables.Input]tables.InputAttr{
					{InputId: 1_00_00_00, PlanId: Padding36("plan"), Path: "/in/1"}: {
						UserTag: []kdb.Tag{{Key: "unexpected", Value: "tag!"}},
					},
				},
			},
			when: []int{1_00_00_00},
	...
	given := tables.Operation{
		...
		Inputs: map[tables.Input]tables.InputAttr{
			...
			{InputId: 101, PlanId: Padding36("plan-1"), Path: "/in/2"}: {
				UserTag: []kdb.Tag{{Key: "tagkey", Value: "tagval"}},
			},
		...
		Steps: []tables.Step{
			{
				Run: tables.Run{
					RunId: Padding36("run-1"), PlanId: Padding36("pseudo"), Status: kdb.Done, UpdatedAt: TIMESTAMP,
				},
				Outcomes: map[tables.Data]tables.DataAttibutes{
					{
			...
			{
				Run: tables.Run{
					RunId: Padding36("run-2"), PlanId: Padding36("pseudo"), Status: kdb.Done, UpdatedAt: TIMESTAMP,
				},
				Outcomes: map[tables.Data]tables.DataAttibutes{
					{
			...
			{
				Run: tables.Run{
					RunId: Padding36("run-3"), PlanId: Padding36("pseudo"), Status: kdb.Done, UpdatedAt: TIMESTAMP,
				},
				Outcomes: map[tables.Data]tables.DataAttibutes{
					{
			...
			{
				Run: tables.Run{
					RunId: Padding36("run-4"), PlanId: Padding36("pseudo"), Status: kdb.Done, UpdatedAt: TIMESTAMP,
				},
				Outcomes: map[tables.Data]tables.DataAttibutes{
					{
