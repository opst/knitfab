...
func Test_ChangingStatus_FromDeactivated(t *testing.T) {
	...
	for _, testcase := range []struct {
	...
	}{
		{
			when: []When{
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-deactivated-1")],
					Cursor: domain.RunCursor{
						Status:     []domain.KnitRunStatus{domain.Deactivated},
						Pseudo:     []domain.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-deactivated-X"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-deactivated-2")],
					Cursor: domain.RunCursor{
						Status:     []domain.KnitRunStatus{domain.Deactivated},
						Pseudo:     []domain.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-deactivated-1"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@image-deactivated-1")],
					Cursor: domain.RunCursor{
						Status:   []domain.KnitRunStatus{domain.Deactivated},
						Head:     th.Padding36("run@image-deactivated-X"),
						Debounce: time.Hour,
					},
				},
			},
			change: []Expectation{
				{
					Statuses: []domain.KnitRunStatus{
						domain.Waiting, domain.Deactivated, domain.Aborting,
					},
					Assertion: CanBeChanged,
				},
				{
					Statuses: []domain.KnitRunStatus{
						domain.Ready, domain.Starting, domain.Running, domain.Completing, domain.Failed, domain.Done, domain.Invalidated,
					},
