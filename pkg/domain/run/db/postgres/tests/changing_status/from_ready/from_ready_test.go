...
func Test_ChangingStatus_FromReady(t *testing.T) {
	...
	for _, testcase := range []struct {
	...
	}{
		{
			when: []When{
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-ready-1")],
					Cursor: domain.RunCursor{
						Status:     []domain.KnitRunStatus{domain.Ready},
						Pseudo:     []domain.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-ready-X"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@pseudo-ready-2")],
					Cursor: domain.RunCursor{
						Status:     []domain.KnitRunStatus{domain.Ready},
						Pseudo:     []domain.PseudoPlanName{PseudoActive, PseudoInactive},
						PseudoOnly: true,
						Head:       th.Padding36("run@pseudo-ready-1"),
						Debounce:   time.Hour,
					},
				},
				{
					Target: given.ExpectedRun[th.Padding36("run@image-ready-1")],
					Cursor: domain.RunCursor{
						Status:   []domain.KnitRunStatus{domain.Ready},
						Head:     th.Padding36("run@image-ready-X"),
						Debounce: time.Hour,
					},
				},
			},
			change: []Expectation{
				{
					Statuses: []domain.KnitRunStatus{
						domain.Ready, domain.Starting, domain.Running, domain.Aborting, domain.Completing,
					},
					Assertion: CanBeChanged,
				},
				{
					Statuses: []domain.KnitRunStatus{
						domain.Waiting, domain.Deactivated, domain.Failed, domain.Done, domain.Invalidated,
					},
