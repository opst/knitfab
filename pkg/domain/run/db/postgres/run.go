...
func (m *runPG) NewPseudo(
	...
	if err := m.setStatus(ctx, tx, runId, domain.Running, 0); err != nil {
		return "", err
	}
...
func (m *runPG) New(ctx context.Context) ([]string, *domain.ProjectionTrigger, error) {
	tx, err := m.pool.Begin(ctx)
	if err != nil {
		return nil, nil, err
	}
...
func (m *runPG) finish(ctx context.Context, tx kpool.Tx, runId string) error {
	...
	var runStatusShouldBe domain.KnitRunStatus = domain.Failed
	switch d := domain.KnitRunStatus(runStatus); d {
	case domain.Completing:
		runStatusShouldBe = domain.Done
	case domain.Aborting:
		runStatusShouldBe = domain.Failed
	default:
		return fmt.Errorf(
			`%w: run (id='%s', status='%s') has not started and stopped`,
			domain.NewErrInvalidRunStateChanging(d, domain.Done),
			runId, d,
		)
	}
	...
	if runStatusShouldBe == domain.Done {
...
func (m *runPG) find(ctx context.Context, conn kpool.Conn, query domain.RunFindQuery) ([]string, error) {
...
func (m *runPG) Find(ctx context.Context, query domain.RunFindQuery) ([]string, error) {
...
func (m *runPG) Get(ctx context.Context, runId []string) (map[string]domain.Run, error) {
	if len(runId) == 0 {
...
func (m *runPG) register(
	...
	if err := conn.QueryRow(
		...
		`,
		domain.Running,
		domain.Waiting, domain.Deactivated,
		planId,
	).Scan(&runId); err != nil {
		return "", xe.Wrap(err)
	}
...
func (m *runPG) SetStatus(ctx context.Context, runId string, newRunStatus domain.KnitRunStatus) error {
	tx, err := m.pool.Begin(ctx)
	if err != nil {
		return err
	}
...
func (m *runPG) setStatus(
	...
	newRunStatus domain.KnitRunStatus, debounceIfNotChanged time.Duration,
) error {
	var current domain.KnitRunStatus
	{
		...
		current = domain.KnitRunStatus(_current)
	}

	allowed := false
	switch current {
	case domain.Invalidated, domain.Done, domain.Failed:
		allowed = false
	case newRunStatus:
		if _, err := tx.Exec(
			ctx,
	...
	case domain.Deactivated:
		switch newRunStatus {
		case domain.Deactivated, domain.Waiting, domain.Aborting:
			allowed = true
		}
	case domain.Waiting:
		switch newRunStatus {
		case domain.Deactivated, domain.Waiting, domain.Ready, domain.Aborting:
			allowed = true
		}
	case domain.Ready:
		switch newRunStatus {
		case domain.Ready, domain.Starting, domain.Running, domain.Aborting, domain.Completing:
			allowed = true
		}
	case domain.Starting:
		switch newRunStatus {
		case domain.Starting, domain.Running, domain.Aborting, domain.Completing:
			allowed = true
		}
	case domain.Running:
		switch newRunStatus {
		case domain.Running, domain.Aborting, domain.Completing:
			allowed = true
		}
	case domain.Aborting:
		if newRunStatus == domain.Failed {
	...
	if !allowed {
		return fmt.Errorf(
			"%w: %s -> %s",
			domain.ErrInvalidRunStateChanging, current, newRunStatus,
		)
	}
...
func (m *runPG) PickAndSetStatus(
	...
	task func(r domain.Run) (domain.KnitRunStatus, error),
...
func (m *runPG) SetExit(ctx context.Context, runId string, exit domain.RunExit) error {
	tx, err := m.pool.Begin(ctx)
	if err != nil {
		return err
	}
...
func (r *runPG) Retry(ctx context.Context, runId string) error {
	...
	default:
		return fmt.Errorf(
			...
			domain.ErrInvalidRunStateChanging, runId, status,
		)
	}
...
func (m *runPG) truncateRun(
	...
	if 0 < downstreams {
		return fmt.Errorf("%w: runId = %s", domain.ErrRunHasDownstreams, runId)
	}
