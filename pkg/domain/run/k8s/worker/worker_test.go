...
func TestWorkerRunning(t *testing.T) {
	...
	kcluster, clientset := testenv.NewCluster(t)
	...
	{ // assert precondition
		...
		_, err = worker.Find(ctx, kcluster, run)
		if err == nil || !kubeerr.IsNotFound(err) {
			t.Fatal(err)
		}
	}
	...
	t.Run("when it spawns worker,", func(t *testing.T) {
		...
		testee := try.To(worker.Spawn(ctx, kcluster, conf, executable)).OrFatal(t)
		...
		t.Run("the worker represents the k8s job", func(t *testing.T) {
			...
			if got := testee.JobStatus(ctx); got.Type == cluster.Failed {
		...
		t.Run("the worker is found and get running", func(t *testing.T) {
			before := time.Now()
			for {
				found := try.To(worker.Find(ctx, kcluster, run)).OrFatal(t)

				got := found.JobStatus(ctx)
				if got.Type == cluster.Running {
					return // ok!
				}
...
func TestWorkerStoppedInSuccess(t *testing.T) {
	...
	workerNewlySpawned := try.To(worker.Spawn(ctx, kcluster, conf, executable)).OrFatal(t)
	...
	t.Run("after worker get be done, it should has exitcode 0", func(t *testing.T) {
		...
		for {
			testee = try.To(worker.Find(ctx, kcluster, executable.RunBody)).OrFatal(t)
			got := testee.JobStatus(ctx)
			if got.Type != cluster.Succeeded {
				time.Sleep(50 * time.Millisecond)
				continue
			}
			if got.Code != 0 {
