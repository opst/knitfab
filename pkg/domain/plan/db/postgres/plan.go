...
func (m *planPG) Get(ctx context.Context, planId []string) (map[string]*types.Plan, error) {
...
func (m *planPG) Register(ctx context.Context, plan *types.PlanSpec) (string, error) {
	...
	if found, ok := utils.First(
		...
		return "", xe.Wrap(types.NewErrEquivPlanExists(found.PlanId))
	}
...
func (m *planPG) activate(ctx context.Context, tx kpool.Tx, planId string, activenessToBe bool) error {
	...
	statusAfterUpdated, statusToBeUpdated := types.GetRunStatusesForPlanActivate(activenessToBe)
...
func (m *planPG) getPlansByHash(ctx context.Context, conn kpool.Queryer, hash string) ([]types.Plan, error) {
...
//
// - error
func registerPlan(ctx context.Context, tx kpool.Tx, plan *types.PlanSpec) (string, mountpointIds, error) {
	if err := plan.Validate(); err != nil {
	...
	//    - error
	insertPlan := func(plan *types.PlanSpec) (planId string, err error) {
		...
		resourceValues := make([]kpgintr.ResourceQuantity, nResource)
		{
			i := 0
			for typ, res := range plan.Resources() {
				resourceTypes[i] = typ
				resourceValues[i] = kpgintr.ResourceQuantity(res)
				i += 1
			}
		}
		if err := tx.QueryRow(
	...
	//     - error
	insertInput := func(planId string, path string, tags *types.TagSet) (int, error) {
		var mpid int
		if err := tx.QueryRow(
			ctx,
			`
		...
		for _, tag := range tags.Slice() {
			switch {
			case tag.Key == types.KeyKnitTransient:
				return 0, types.NewErrUnacceptableTag(
					path, `data with "knit#transient" is never used for input tag`,
				)
			case tag.Key == types.KeyKnitId:
				if _, err := tx.Exec(
					ctx,
				...
				continue
			case tag.Key == types.KeyKnitTimestamp:
				t, _err := rfctime.ParseRFC3339DateTime(tag.Value)
				if _err != nil {
					return 0, types.NewErrBadFormatKnitTimestamp(tag.Value)
				}
				if _, err := tx.Exec(
					ctx,
				...
				continue
			case strings.HasPrefix(tag.Key, types.SystemTagPrefix):
				return 0, types.NewErrUnknownSystemTag(&tag)
			}
	...
	//     - *kdb.Mountpoint: new mountpoint
	//     - error
	insertOutput := func(planId string, path string, forLog bool, tags *types.TagSet) (int, error) {
		var mpid int
		if err := tx.QueryRow(
			ctx,
			`
		...
		// set tags.
		for _, tag := range tags.Slice() {
			if strings.HasPrefix(tag.Key, types.SystemTagPrefix) {
				return 0, fmt.Errorf(
					"%s: %w",
					tag, types.NewErrUnacceptableTag(path, "cannot put system tags on output"),
				)
			}
...
func (m *planPG) SetResourceLimit(ctx context.Context, planId string, resource map[string]resource.Quantity) error {
	...
	types := []string{}
	quants := []kpgintr.ResourceQuantity{}
	for typ, val := range resource {
		types = append(types, typ)
		quant := kpgintr.ResourceQuantity(val)
...
func planDependencyIsCyclic(ctx context.Context, conn kpool.Queryer, plan string) error {
	...
	findPlanIdInDependants := func(origin string, from []string) ([]string, error) {
		...
		for ret.Next() {
			...
			if dependant == origin {
				return nil, xe.Wrap(types.NewErrCyclicPlan())
			}
...
func (m *planPG) Find(ctx context.Context, active logic.Ternary, imageVer types.ImageIdentifier, inTag []types.Tag, outTag []types.Tag) ([]string, error) {
...
func (m *planPG) makeTagQuery(inTag []types.Tag, outTag []types.Tag) *findTagQuery {
	...
	trimSpace := func(tag types.Tag) types.Tag {
		return types.Tag{
...
func (m *planPG) UpdateAnnotations(ctx context.Context, planId string, delta types.AnnotationDelta) error {
	tx, err := m.pool.Begin(ctx)
	if err != nil {
		return err
	}
