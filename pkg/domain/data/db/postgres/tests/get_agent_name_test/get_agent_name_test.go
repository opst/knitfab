...
func TestGetAgentName(t *testing.T) {
	...
	given := tables.Operation{
		...
		Steps: []tables.Step{
				...
				Outcomes: map[tables.Data]tables.DataAttibutes{
					{
						...
						Agent: []tables.DataAgent{
							...
							{
								Name: "test-agent-2", Mode: domain.DataAgentRead.String(),
								KnitId: Padding36("test-knit-running-1"),
								LifecycleSuspendUntil: try.To(rfctime.ParseRFC3339DateTime(
							...
							{
								Name: "test-agent-3", Mode: domain.DataAgentRead.String(),
								KnitId: Padding36("test-knit-running-1"),
								LifecycleSuspendUntil: try.To(rfctime.ParseRFC3339DateTime(
							...
							{
								Name: "test-agent-4", Mode: domain.DataAgentWrite.String(),
								KnitId: Padding36("test-knit-running-1"),
								LifecycleSuspendUntil: try.To(rfctime.ParseRFC3339DateTime(
			...
			{
				Run: tables.Run{
					PlanId: "test-plan", RunId: "test-run-running-3", Status: domain.Done,
					UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
						"2022-10-11T12:13:14.567+09:00",
					)).OrFatal(t).Time(),
				},
			...
			{
				Run: tables.Run{
					PlanId: "test-plan", RunId: "test-run-running-4", Status: domain.Done,
					UpdatedAt: try.To(rfctime.ParseRFC3339DateTime(
						"2022-10-11T12:13:14.567+09:00",
					)).OrFatal(t).Time(),
				},
	...
	t.Run("when there is no such data, it returns Missing error", theory(
		When{
			...
			Mode:   []domain.DataAgentMode{domain.DataAgentRead, domain.DataAgentWrite},
		},
		Then{
			Names: []string{},
	...
	t.Run("when there is an agent with specified mode (read), it returns the name", theory(
		When{
			KnitId: Padding36("test-knit-running-1"),
			Mode:   []domain.DataAgentMode{domain.DataAgentRead},
		},
		Then{
	...
	t.Run("when there is an agent with specified mode (write), it returns the name", theory(
		When{
			KnitId: Padding36("test-knit-running-1"),
			Mode:   []domain.DataAgentMode{domain.DataAgentWrite},
		},
		Then{
