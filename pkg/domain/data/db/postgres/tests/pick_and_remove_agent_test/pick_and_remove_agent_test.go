...
func TestPickAndRemoveAgent(t *testing.T) {
	...
	theory := func(given Given, when When, then Then) func(*testing.T) {
		return func(t *testing.T) {
			...
			actualCursor, err := testee.PickAndRemoveAgent(
				ctx, when.Cursor,
				func(da domain.DataAgent) (bool, error) {
					called = true
	...
	t.Run("Let DataAgents are given, it picks a unsuspended DataAgent named next of Cursor Head, and remove", theory(
		Given{
			Agents: []RecordSpec{
				{KnitId: knitId, Name: "knitid-test-knit-done-read-12", Mode: domain.DataAgentRead, Debounce: -30 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-14", Mode: domain.DataAgentRead, Debounce: -20 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-16", Mode: domain.DataAgentRead, Debounce: -10 * time.Second}, // ^^^ past
				{KnitId: knitId, Name: "knitid-test-knit-done-read-18", Mode: domain.DataAgentRead, Debounce: 10 * time.Second},  // vvv future
				{KnitId: knitId, Name: "knitid-test-knit-done-read-20", Mode: domain.DataAgentRead, Debounce: 20 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-22", Mode: domain.DataAgentRead, Debounce: 30 * time.Second},
			},
		},
		When{
			Cursor: domain.DataAgentCursor{
			...
			DataAgent: domain.DataAgent{
				...
				KnitDataBody: domain.KnitDataBody{
					...
					Tags: domain.NewTagSet([]domain.Tag{
						...
						{Key: domain.KeyKnitTimestamp, Value: "2022-10-11T12:13:15.567+09:00"},
					}),
				},
			},
			Cursor: domain.DataAgentCursor{
	...
	t.Run("Let DataAgents are given and some records are locked, it picks a unsuspended & unlocked DataAgent named next of Cursor Head, and remove", theory(
		...
		Then{
			...
			Records: []RecordSpec{
				...
				{KnitId: knitId, Name: "knitid-test-knit-done-read-18", Mode: domain.DataAgentRead, Debounce: 10 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-20", Mode: domain.DataAgentRead, Debounce: 20 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-22", Mode: domain.DataAgentRead, Debounce: 30 * time.Second},
			},
		},
	...
	t.Run("Let DataAgents are given but all of them are suspended, it does not pick", theory(
		Given{
			Agents: []RecordSpec{
				{KnitId: knitId, Name: "knitid-test-knit-done-read-12", Mode: domain.DataAgentRead, Debounce: 30 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-14", Mode: domain.DataAgentRead, Debounce: 20 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-16", Mode: domain.DataAgentRead, Debounce: 10 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-18", Mode: domain.DataAgentRead, Debounce: 10 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-20", Mode: domain.DataAgentRead, Debounce: 20 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-22", Mode: domain.DataAgentRead, Debounce: 30 * time.Second},
			},
	...
	t.Run("Let DataAgents are given, it picks a unsuspended DataAgent named next of Cursor Head, and suspend when it is not deleted", theory(
		...
		Then{
			...
			Records: []RecordSpec{
				{KnitId: knitId, Name: "knitid-test-knit-done-read-12", Mode: domain.DataAgentRead, Debounce: -30 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-14", Mode: domain.DataAgentRead, Debounce: time.Hour},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-16", Mode: domain.DataAgentRead, Debounce: -10 * time.Second}, // ^^^ past
				{KnitId: knitId, Name: "knitid-test-knit-done-read-18", Mode: domain.DataAgentRead, Debounce: 10 * time.Second},  // vvv future
				{KnitId: knitId, Name: "knitid-test-knit-done-read-20", Mode: domain.DataAgentRead, Debounce: 20 * time.Second},
				{KnitId: knitId, Name: "knitid-test-knit-done-read-22", Mode: domain.DataAgentRead, Debounce: 30 * time.Second},
