...
func TestData_Find(t *testing.T) {
	...
	tagsetAandC := []types.Tag{tagsetAll[0], tagsetAll[2]}
	...
	t.Run("(matrix test)", func(t *testing.T) {
		...
		for tagcode, tag := range map[string][]types.Tag{
			"a-and-b": tagsetAandB,
			"b-and-c": tagsetBandC,
		...
		for name, testcase := range map[string]struct {
			...
			`when querying by "knit#transient: failed", it returns data come from Failed run`: {
				when{
					tags: []types.Tag{
						try.To(types.NewTag(types.KeyKnitTransient, types.ValueKnitTransientFailed)).OrFatal(t),
					},
				},
				then{
					knitId: []string{
			...
			`when querying by "knit#transient: processing", it returns data come from unterminated run`: {
				when{
					tags: []types.Tag{
						try.To(types.NewTag(types.KeyKnitTransient, types.ValueKnitTransientProcessing)).OrFatal(t),
					},
				},
				then{
					knitId: []string{
			...
			`when querying by "knit#timestamp: ...", it returns data having timestamp at same time`: {
				when{
					tags: []types.Tag{
						types.NewTimestampTag(oldTimestamp),
					},
				},
				then{
					knitId: []string{
			...
			`when querying by "knit#timestamp: ..." but there are no such data, it returns nothing`: {
				when{
					tags: []types.Tag{
						types.NewTimestampTag(try.To(rfctime.ParseRFC3339DateTime(
			...
			`when querying by different "knit#timestamp: ..."s, it returns nothing`: {
				when{
					tags: []types.Tag{
						types.NewTimestampTag(newTimestamp),
						types.NewTimestampTag(oldTimestamp),
					},
				},
				then{knitId: []string{}}, // empty!
			...
			`when querying by "knit#id: ..." but no such data, it returns nothing`: {
				when{
					tags: []types.Tag{
						{Key: types.KeyKnitId, Value: Padding36("there-are-not-such-one")},
					},
				},
				then{knitId: []string{}},
			},
			`when querying by different "knit#id: ...", it returns nothing`: {
				when{
					tags: []types.Tag{
						{Key: types.KeyKnitId, Value: Padding36("knit-a-and-b-new-done")},
						{Key: types.KeyKnitId, Value: Padding36("knit-a-and-b-old-done")},
					},
				},
				then{knitId: []string{}},
			},
	...
	t.Run("", func(t *testing.T) {
		...
		for tagcode, tag := range map[string][]types.Tag{
			...
			for timecode, timestamp := range times {
				var statuses []types.KnitRunStatus
				if tagcode == "a-and-b" {
					statuses = []types.KnitRunStatus{
					...
					statuses = []types.KnitRunStatus{
						...
						types.Done, types.Failed, types.Invalidated,
					}
				}
	...
	t.Run(`let given runs are succeeded`, func(t *testing.T) {
		...
		for tagcode, tag := range map[string][]types.Tag{
			...
			for timecode, timestamp := range map[string]*time.Time{
				...
				for _, status := range []types.KnitRunStatus{types.Done} {
					// status in id are cut off after the 4th letter,
					// to maintain ids shorter than 36 chars.
					runid := Padding36(fmt.Sprintf(
