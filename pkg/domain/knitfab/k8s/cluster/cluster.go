...
type Cluster interface {
	...
	// - context.Context
	//
	// - backoff retry.Backoff: backoff policy to wait for PVC deletion.
	//
	// - string: name of PVC
	//
	// Return
	//
	// - error: error if any
	DeletePVC(context.Context, retry.Backoff, string) retry.Promise[struct{}]
...
func WithCheckpoint[T any](requirement Requirement[T], deadline time.Time) Requirement[T] {
	...
	return func(value T) error {
		...
		if time.Now().After(deadline) {
			return k8serrors.ErrDeadlineExceeded
		}
...
func (c *k8sCluster) NewService(
	...
	if err != nil {
		if kubeerr.IsAlreadyExists(err) {
			return retry.Failed[Service](k8serrors.NewConflictCausedBy("", err))
		}
...
func (c *k8sCluster) NewDeployment(
	...
	if err != nil {
		if kubeerr.IsAlreadyExists(err) {
			return retry.Failed[Deployment](k8serrors.NewConflictCausedBy("", err))
		}
...
func (c *k8sCluster) NewPVC(
	...
	if err != nil {
		if kubeerr.IsAlreadyExists(err) {
			return retry.Failed[PVC](k8serrors.NewConflictCausedBy("", err))
		}
	...
	return retry.Go(ctx, backoff, func() (PVC, error) {
		...
		if err != nil {
			if kubeerr.IsNotFound(err) {
				return ret, k8serrors.NewMissingCausedBy("", err)
			}
			return ret, err
		}
...
func (c *k8sCluster) DeletePVC(ctx context.Context, backoff retry.Backoff, name string) retry.Promise[struct{}] {
	err := c.client.DeletePVC(ctx, c.namespace, name)
	if err != nil {
		if kubeerr.IsNotFound(err) {
			return retry.Ok(struct{}{})
		}
		return retry.Failed[struct{}](err)
	}
	return retry.Go(ctx, backoff, func() (struct{}, error) {
		_, err := c.client.GetPVC(ctx, c.namespace, name)
		if err == nil {
			return struct{}{}, retry.ErrRetry
		}
		if kubeerr.IsNotFound(err) {
			return struct{}{}, nil
		}
...
func (c *k8sCluster) NewJob(
	...
	if err != nil {
		if kubeerr.IsAlreadyExists(err) {
			return retry.Failed[Job](k8serrors.NewConflictCausedBy("", err))
		}
...
func (c *k8sCluster) NewPod(
	...
	if err != nil {
		if kubeerr.IsAlreadyExists(err) {
			return retry.Failed[Pod](k8serrors.NewConflictCausedBy("", err))
		}
