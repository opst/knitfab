...
import (
	...
	"github.com/opst/knitfab/v2/pkg/domain/internal/db/postgres"
...
func TestOperation(t *testing.T) {
	...
	testee := tables.Operation{
		...
		PlanResources: []tables.PlanResource{
			{PlanId: th.Padding36("plan-1"), Type: "cpu", Value: postgres.ResourceQuantity(resource.MustParse("1"))},
			{PlanId: th.Padding36("plan-2"), Type: "memory", Value: postgres.ResourceQuantity(resource.MustParse("500Mi"))},
		},
		OnNode: []tables.PlanOnNode{
			{PlanId: th.Padding36("plan-1"), Mode: domain.MayOnNode, Key: "key1", Value: "value1"},
			{PlanId: th.Padding36("plan-2"), Mode: domain.MustOnNode, Key: "key2", Value: "value2"},
			{PlanId: th.Padding36("plan-2"), Mode: domain.PreferOnNode, Key: "key2", Value: "value2"},
		},
		...
		PlanEntrypoint: []tables.PlanEntrypoint{
			{PlanId: th.Padding36("plan-3"), Entrypoint: []string{"python", "main.py"}},
		},
		PlanArgs: []tables.PlanArgs{
			{PlanId: th.Padding36("plan-4"), Args: []string{"--arg1", "val1", "--arg2", "val2"}},
		},
		Outputs: map[tables.Output]tables.OutputAttr{
			...
			{OutputId: 3030, PlanId: th.Padding36("plan-3"), Path: "/out/3"}: {
				UserTag: []domain.Tag{}, // no user tags
			},
				...
				Outcomes: map[tables.Data]tables.DataAttibutes{
					{
						...
						Agent: []tables.DataAgent{
							...
							},
							{
								...
								Mode:   domain.DataAgentWrite.String(),
								KnitId: th.Padding36("data-1.run-1.plan-2"),
								LifecycleSuspendUntil: try.To(rfctime.ParseRFC3339DateTime(
									"2021-10-11T12:13:15.567+09:00",
								)).OrFatal(t).Time(),
	...
	t.Run("plan_entrypoint", func(t *testing.T) {
		...
		expected := testee.PlanEntrypoint
		if !cmp.SliceContentEqWith(actual, expected, tables.PlanEntrypoint.Equal) {
			t.Errorf("unmatch:\n===actual===\n%+v\n===expected===\n%+v", actual, expected)
		}
	})
	...
	t.Run("plan_args", func(t *testing.T) {
		...
		expected := testee.PlanArgs
		if !cmp.SliceContentEqWith(actual, expected, tables.PlanArgs.Equal) {
			t.Errorf("unmatch:\n===actual===\n%+v\n===expected===\n%+v", actual, expected)
		}
	})
