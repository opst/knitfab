...
出力のタグは「その出力に書き出された "データ" には、どういう "タグ" を指定すべきか」を示している。
...
各入力に何らかの "データ" が割り当てられた "プラン" があることを Knitfab が犬種すると、Knitfab は自動的に、入力と "データ" の組み合わせから "ラン" を生成する。
...
- `tag`: "データ" や "プラン" 入出力に標準的にセットするタグ (複数設定可。デフォルトは空)
...
そうやって Knitfab に蓄積された "データ" は、 "プラン" の入力と突き合わせられて、実行可能な "プラン" が見つかれば "ラン" として実行されることになる。
...
フラグ `-t` は「この "タグ" をもっていること」を検索条件として指定するものであるが、ここに `knit#id:` タグの値として "データ" の ID を指定した。結果として、特定の "データ" についてメタデータを得ることができる。
...
`on_node` に適当な値を設定することで、このプランのランを配置する計算機を制限できる。
...
ランが成功したかどうかは、ユーザ定義コンテナの終了ステータスによって決まる。
...
- `inputs`, `outputs`, `log`: この "ラン" の実際の入出力とログ。
    ...
    - `tags` はその入出力やログの "タグ" である ( "データ" のタグではない) 。
...
もし "ラン" の ID が特定できているのであれば、 `knit run show RUN_ID` というコマンドをつかっても、 "ラン" の情報を確認できる。
...
フラグ `--fail` 省略すると、 "ラン" は中断したうえに成功したものとして扱われる。こちらは状態を `completing` にすることによる。
...
ということをする。`waiting` に差し戻された "ラン" は、ライフサイクルにしたがって改めて実行される。

他の "ラン" が入力として使っている "データ" を出力した "ラン" はリトライできない。
...
**"ラン" がすでに停止していて、下流に他の "ラン" がない場合に限り**、ランを削除できる。
