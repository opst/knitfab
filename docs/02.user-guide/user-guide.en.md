...
The automatic lineage management feature automatically records the generation history of all data generated by Knitfab by tracking the relationship between each task and its inputs and outputs during the execution of the tasks.
...
- Inputs are read from the file system (directory).
- Outputs are written to standard output/error or to the file system (directory).
...
`knit` is divided into subcommands based on the concepts it handles.
...
This file is intended to be shared among the team.
...
If you want to use a collection of files, such as a dataset, as input for a machine learning task, you can register it as Data in Knitfab.
...
- `upstream`: It shows the output where this Data was written and information about the corresponding Run.
    - In this case, it displays a special Run of a Plan that accepts the upload of Data.
...
If `-t` is not specified at all, all Data will be searched.
...
- `outputs`: Specifies "what outputs" can be obtained.
...
These search conditions can be specified simultaneously, and only the Plans that meet all the conditions will be matched in the search.
...
> If multiple Runs are found, the elements will be in an array.
